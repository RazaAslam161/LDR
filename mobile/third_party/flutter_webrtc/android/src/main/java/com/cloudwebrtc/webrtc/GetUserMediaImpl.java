                ...
                new OrientationAwareScreenCapturer(
                        ...
                        new MediaProjection.Callback() {
                            ...
                            public void onStop() {
                                ...
                                //
                                // Miles patch: the system cast notification's "Stop
                                // sharing" lands HERE and upstream drops it on the
                                // floor — the app side then watches a frozen frame
                                // until a stall heuristic guesses. Forward it as a
                                // plugin event instead. This callback runs on the
                                ...
                                new Handler(Looper.getMainLooper()).post(() -> {
                                    if (FlutterWebRTCPlugin.sharedSingleton != null) {
                                        ...
                                        params.putString("event", "miles.screenCaptureStop");
                                        FlutterWebRTCPlugin.sharedSingleton.sendEvent(params.toMap());
                                    }
                                });
                            }

                            // Miles patch: Android 14+ reports the CONTENT size of
                            // the capture — which is also the only signal that the
                            // user chose a single APP rather than the whole display.
                            // Never called before API 34; overriding is harmless.
                            @Override
                            public void onCapturedContentResize(int width, int height) {
                                new Handler(Looper.getMainLooper()).post(() -> {
                                    if (FlutterWebRTCPlugin.sharedSingleton != null) {
                                        ConstraintsMap params = new ConstraintsMap();
                                        params.putString("event", "miles.screenCaptureContentResize");
