...
| **Private Vault notes and labels** | `personal_vault_items.content`, `media_url` (repurposed as the display label) | **Owner + operator** | Written in the clear by `VaultRepository.addNote`. As of build 60 the vault's files are not encrypted either, so the old files-vs-text distinction is gone: nothing in the Personal Vault is E2EE. The 2026-08-30 pass corrected the privacy policy, the security page's own table and the in-app FAQ's encryption answer — but **it missed five places, and this line claimed otherwise for five days**: the in-app Terms (`terms_text.dart`, the document the gate makes every user accept), `closer_screen.dart`'s waiting-for-partner copy, `csae.html` §4, `security.html` §3's bug-bounty scope, `index.html`'s security block, the runbook's Data safety answer, and the in-app FAQ's break-up answer ("sealed with a key derived from your own account"). All corrected 2026-09-04, and the Terms version bumped 1 -> 2 so the correction is re-accepted rather than applying to new users only. Do not write "corrected everywhere" into this table again without grepping for the claim first. |
