// care-notify — Care Nudge push (eat/meds/water/sleep reminders). FCM HTTP v1.
// Invoked by a Database Webhook on INSERT to public.care_nudges. Sends a DATA
// message (type=care) so the app builds a gentle reminder notification.
...
Deno.serve(async (req) => {
  try {
    ...
    const nudgeId: string = row?.id ?? "";
    const message: string = row?.message ?? "A little reminder";
    if (!fromUser || !coupleId) {
      return new Response(JSON.stringify({ error: "bad payload" }), { status: 400 });
    }

    const { data: recipients } = await admin.from("profiles").select("id, fcm_token").eq("couple_id", coupleId).neq("id", fromUser).limit(1);
    const recipient = recipients?.[0];
    if (!recipient?.fcm_token) {
      return new Response(JSON.stringify({ skipped: "no recipient token" }), { status: 200 });
    }

    const { data: sender } = await admin.from("profiles").select("display_name").eq("id", fromUser).single();
    ...
    const fcmMessage = {
      message: {
        ...
        data: { type: "care", title: `${fromName} 💛`, body: message, nudge_id: nudgeId, from_name: fromName },
    ...
    if (!fcmRes.ok) {
      ...
      console.error("care FCM send failed", fcmRes.status, errText);
      return new Response(JSON.stringify({ error: errText }), { status: 200 });
    }
    return new Response(JSON.stringify({ ok: true }), { status: 200 });
  } catch (e) {
    console.error("care-notify error", e);
